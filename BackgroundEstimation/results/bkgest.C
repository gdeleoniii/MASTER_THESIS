...
void backest(std::string var,std::string name){
  TFile* file = TFile::Open("JetHT-Run2015_bkgest.7.0.2.root");
  ...
  */
  ...
  TLegend *leg = new TLegend(0.56, 0.80, 0.79, 0.88);
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->SetTextSize(0.03);
  leg->SetHeader("Anti-#tau_{21} region");
  leg->AddEntry(h1, "one anti-tag (sub-lead fails)", "lp");
  leg->AddEntry(h2, "two anti-tag", "l");
  leg->Draw();
  
  c4->Print(Form("%s_bkgest.7.0.2.pdf",var.data()));
}
